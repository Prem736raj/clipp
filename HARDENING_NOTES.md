...
- Export now renders basic constant speed (with pitch preservation option), rotation, horizontal/vertical flip, clip positioning, transform keyframes (position/scale/rotation), per-clip volume, mute, trims, still-image durations, crop, supported filters/adjustments, Gaussian blur, static text/captions/stickers/drawings/frames, image overlays, fade-to-black/white, and separate audio tracks through Media3 effects/audio processors. Crop-animation keyframes, animated effects, video overlays, unsupported transitions, and advanced audio automation remain blocked.
...
- Added a device export regression using a generated local image, then re-exporting the resulting MP4 as a video clip with speed/rotation/flip: Media3 rendering, MediaStore publication, output reopening, non-zero size, and positive duration all pass on `small_phone` API 36. A real user-selected video-file and multi-video duration/order check remain pending.
