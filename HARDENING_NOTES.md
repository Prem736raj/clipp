...
- Added a device export regression using a generated local image: Media3 rendering, MediaStore publication, output reopening, non-zero size, and positive duration all pass on `small_phone` API 36. A real user-selected video-file export and multi-video duration/order check remain pending.
